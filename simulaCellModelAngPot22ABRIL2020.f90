...
!
  ...
  character :: nomexmake*60, nomeenergia*60, nomeparaview*60,leitura_nomexmake*160,path*100
 ...
 do isig=1,1
  do jepi=0,0
   do lmas=2,10,1
        ...
        path='inputData/'
!        leitura_nomexmake=TRIM(path)//'saidaxmakemol_isig_000001_iepi_000003_lmas_000005_.xyz'
...
kmolaext=1000.D0     ! constante de mola de puxamento
...
SUBROUTINE define_atomo_puxa()
 ...
 integer(kind=8) :: i,j
 real (kind=8) :: dx,dy,dz,rtest,xmax
 integer(kind=8),dimension(100):: selected_particles
 
tpuxar=2*dt
!! seleciona o atomo que puxo.

selected_particles=0
!
iext=4430+lmas!selected_particles(1)
selected_particles(1)=iext
x0mola=atomo(iext)%pos%x
...
! SUB-ROTINA saidaxmakemol
!####################################################
SUBROUTINE leitura_saidaxmakemol()
 USE parametros
 USE tipos
 USE variables
 IMPLICIT NONE
 integer(kind=8) :: i,j,it,read_unit,iostatus,n,ios,indx,nbin,iframe
 character :: atvec*13,nometipo*4,line*200,auxchar*200
 character(len=200), allocatable :: command(:)
 integer(kind=8), allocatable :: histlen(:),linelen(:)
 
 nbin=200
 read_unit=411
 atvec=' atom_vector '
 
 open(unit=read_unit, file=leitura_nomexmake, iostat=ios)
    if ( ios /= 0 ) stop "Error opening file "//TRIM(leitura_nomexmake)
    n = 0
    do
    ...
    do i = 1, n
        ...
        linelen(i)=LEN(TRIM(command(i)))
        histlen(linelen(i))=histlen(linelen(i))+1
    end do

    do i=1,200
       if (histlen(i).ne.0) print*, "i=",i,"histlen(i)=",histlen(i)
    end do

    rewind(read_unit)
    READ(read_unit, *, IOSTAT=iostatus) Np
    print*, 'Np=',Np
    allocate(atomo(Np))
    rewind(read_unit)

256 format(A3,3F16.6,A13,3F16.6)!'(A3,3F16.6,A13,3F16.6)'
257 format(A4,3F16.6,A13,3F16.6)!'(A4,3F16.6,A13,3F16.6)'
    
    do i = 1,int(n/(Np+2))
        READ(read_unit, *, IOSTAT=iostatus) Np
        READ(read_unit, *, IOSTAT=iostatus) tempo
!        print*, Np,tempo,i

        iframe=i-1
!        print*,'iframe=',iframe,mod(i,Np+2)-1

        Np1=0
        Np2=0
        do j=1,Np
            SELECT CASE (linelen(iframe*(Np+2)+j+2))
            CASE (112)
                read(read_unit,256,IOSTAT=iostatus) nometipo,atomo(j)%pos%x,atomo(j)%pos%y,atomo(j)%pos%z,atvec,atomo(j)%vel%x,&
                atomo(j)%vel%y,atomo(j)%vel%z
                if (nometipo=='Au  ') then
                 atomo(j)%tipo=0
                 Np1=Np1+1
                elseif (nometipo=='Fe  ') then
                 atomo(j)%tipo=1
                 Np2=Np2+1
                elseif (nometipo=='C  ') then
                 atomo(j)%tipo=3
                 Np2=Np2+1
                end if
                
!                print*, TRIM(command( iframe*(Np+2)+j+2 ))
!                print'(A1,A4,3F16.6,A14,3F16.6)',' ',nometipo,atomo(j)%pos%x,atomo(j)%pos%y,atomo(j)%pos%z,atvec,atomo(j)%vel%x,&
!                atomo(j)%vel%y,atomo(j)%vel%z
!                print*,' '
            CASE (113)
                read(read_unit,257,IOSTAT=iostatus) nometipo,atomo(j)%pos%x,atomo(j)%pos%y,atomo(j)%pos%z,atvec,atomo(j)%vel%x,&
                atomo(j)%vel%y,atomo(j)%vel%z
                if (nometipo=='Au  ') then
                 atomo(j)%tipo=0
                 Np1=Np1+1
...
!                print*, TRIM(command( iframe*(Np+2)+j+2 ))
!                print'(A1,A3,3F16.6,A14,3F16.6)',' ',nometipo,atomo(j)%pos%x,atomo(j)%pos%y,atomo(j)%pos%z,atvec,atomo(j)%vel%x,&
!                atomo(j)%vel%y,atomo(j)%vel%z
!                print*,' '
            END SELECT
    ...
    if (i==1) then
     ...
     call define_atomo_puxa()
     print*,'call define_atomo_puxa() - ok'  
...
call calculaenercin()
write(21,32) tempo, Ecin/Np, Epot/Np, (Ecin+Epot)/Np,forcaMolaexterna_x
close(21)
...
! 
  ...
  ! 
   ...
   if (tempo.lt.tpuxar) then
    x0mola=atomo(iext)%pos%x
    forcaMolaexterna_x=0.0D0
   end if
   if (tempo.gt.tpuxar) then
    gamma=0.10D0
    forcaMolaexterna_x=-kmolaext*(atomo(iext)%pos%x-x0mola)
    atomo(iext)%forca%x=atomo(iext)%forca%x+forcaMolaexterna_x
    x0mola=x0mola+0.5*dt
   end if
